...
const PokecodeGame: React.FC = () => {
  ...
  const handleBattleAttack = () => {
    ...
    if (newWildHealth <= 0) {
      newBattleLog.push(`${wildPokemon.name} fainted!`);
      result = 'WIN';
      
      // Add experience to player's Pokemon
      const expGain = Math.floor(wildPokemon.level * 10 * (1 + Math.random() * 0.2));
    ...
    // If battle is over, handle capture or flee
    if (result === 'WIN') {
      setTimeout(() => {
        const updated = addPokemonToPokedex(wildPokemon);
        setUserData(updated);
      }, 2000);
    }
  };
